"""
LightGBM 模型 D — 假设检验: 时序进化 + 品类交叉 vs 基线 A
==========================================================
A = GROUP_BASE (113维) — 对照组
D = GROUP_BASE + GROUP_TEMPORAL (4) + GROUP_CATEGORY (3) = 120维
"""
import pandas as pd
import numpy as np
...
cols_D = groups["GROUP_BASE"] + groups["GROUP_TEMPORAL"] + groups["GROUP_CATEGORY"]   # 120 维

print(f"  A: {len(cols_A)} 维 (GROUP_BASE)")
print(f"  D: {len(cols_D)} 维 (BASE + TEMPORAL + CATEGORY)")
print(f"  新增特征: {groups['GROUP_TEMPORAL'] + groups['GROUP_CATEGORY']}")
...
X_train_D = train[cols_D]
X_valid_A, y_valid = valid[cols_A], valid["label"]
X_valid_D = valid[cols_D]

time_cols = [c for c in cols_D if "first_time" in c or "last_time" in c]
...
X_train_D = convert_time_cols(X_train_D)
X_valid_A = convert_time_cols(X_valid_A)
X_valid_D = convert_time_cols(X_valid_D)

cat_cols_A = [c for c in cat_cols if c in cols_A]
cat_cols_D = [c for c in cat_cols if c in cols_D]
...
for col in cat_cols_D:
    X_train_D[col] = X_train_D[col].astype("category")
    X_valid_D[col] = X_valid_D[col].astype("category")

print(f"  A 分类特征: {len(cat_cols_A)}  |  D 分类特征: {len(cat_cols_D)}")

# ============================================================
# 3. 训练 LightGBM
# ============================================================
print("\n" + "=" * 60)
print("3. 训练 LightGBM A vs D")
print("=" * 60)
...
print("  训练 LGB-D (BASE+TEMPORAL+CATEGORY, 120维)...")
model_d = lgb.LGBMClassifier(**base_params, categorical_feature=cat_cols_D)
...
proba_d = model_d.predict_proba(X_valid_D)[:, 1]

auc_a = roc_auc_score(y_valid, proba_a)
auc_d = roc_auc_score(y_valid, proba_d)

print(f"  LGB-A (113维): AUC = {auc_a:.4f}")
print(f"  LGB-D (120维): AUC = {auc_d:.4f}")
print(f"  ΔAUC (D - A):  {auc_d - auc_a:+.4f}")

# ============================================================
# 5. 特征重要性 Top-20 (D 模型)
# ============================================================
print("\n" + "=" * 60)
print("5. 特征重要性 Top-20 (模型 D)")
print("=" * 60)
...
new_features = groups["GROUP_TEMPORAL"] + groups["GROUP_CATEGORY"]
imp_d["is_new"] = imp_d["feature"].isin(new_features)
...
plt.title("LGB-D Top-20 特征重要性 (红色=新特征)", fontsize=14)
plt.tight_layout()
plt.savefig(FIGURES_DIR / "lgb_d_importance.png", dpi=150)
plt.close()
print(f"  已保存: lgb_d_importance.png")
...
result_d = compute_mrr_with_scenario(model_d, cols_D, cat_cols_D, df_eval, "D")

print()
print(f"  {'':>18} {'A (113维)':>12} {'D (120维)':>12} {'Δ':>10}")
print(f"  {'─'*52}")
mrr_a_g, hits_a_g = result_a["global"]
mrr_d_g, hits_d_g = result_d["global"]
print(f"  {'全局 MRR@5':>18} {mrr_a_g:>12.5f} {mrr_d_g:>12.5f} {mrr_d_g-mrr_a_g:>+10.5f}")
print(f"  {'全局 HITS@5':>18} {hits_a_g:>12.5f} {hits_d_g:>12.5f} {hits_d_g-hits_a_g:>+10.5f}")
print(f"  {'─'*52}")
mrr_a_f, hits_a_f, n_f = result_a["friend"]
mrr_d_f, hits_d_f, _ = result_d["friend"]
print(f"  {'朋友组 MRR@5':>18} {mrr_a_f:>12.5f} {mrr_d_f:>12.5f} {mrr_d_f-mrr_a_f:>+10.5f}  (n={n_f})")
mrr_a_s, hits_a_s, n_s = result_a["stranger"]
mrr_d_s, hits_d_s, _ = result_d["stranger"]
print(f"  {'陌生人组 MRR@5':>18} {mrr_a_s:>12.5f} {mrr_d_s:>12.5f} {mrr_d_s-mrr_a_s:>+10.5f}  (n={n_s})")
print(f"  {'─'*52}")
print(f"  {'朋友组 HITS@5':>18} {hits_a_f:>12.5f} {hits_d_f:>12.5f} {hits_d_f-hits_a_f:>+10.5f}")
print(f"  {'陌生人组 HITS@5':>18} {hits_a_s:>12.5f} {hits_d_s:>12.5f} {hits_d_s-hits_a_s:>+10.5f}")
...
delta_auc = auc_d - auc_a
delta_mrr_global = mrr_d_g - mrr_a_g
delta_mrr_stranger = mrr_d_s - mrr_a_s
delta_mrr_friend = mrr_d_f - mrr_a_f
...
judgments = []
if delta_auc > 0:
    judgments.append("✅ AUC 提升: D > A")
else:
    judgments.append("❌ AUC 未提升: D ≤ A")
if delta_mrr_global > 0:
    judgments.append("✅ MRR@5 全局提升: D > A")
else:
    judgments.append("❌ MRR@5 全局未提升: D ≤ A")
if delta_mrr_stranger > 0:
    judgments.append("✅ 陌生人组 MRR 提升: 新特征提升了冷启动泛化能力")
else:
...
if delta_mrr_stranger > delta_mrr_friend:
    ...
    judgments.append("⚠️ 朋友组 Δ ≥ 陌生人组 Δ: 新特征可能部分依赖统计记忆")
...
print(f"\n  新特征进入 Top-20: {new_in_top20}/7")
if new_in_top20 >= 2:
    print("  ✅ 多个新特征进入 Top-20: 新特征有实质性贡献")
else:
    print("  ⚠️ 新特征在 Top-20 中占比较少: 增量有限")
...
joblib.dump(model_a, MODEL_DIR / "lgb_baseline_D_A.pkl")
joblib.dump(model_d, MODEL_DIR / "lgb_baseline_D.pkl")
print(f"  已保存: lgb_baseline_D_A.pkl, lgb_baseline_D.pkl")

results = {
    "auc_a": auc_a, "auc_d": auc_d, "delta_auc": delta_auc,
    "mrr_global_a": mrr_a_g, "mrr_global_d": mrr_d_g, "delta_mrr_global": delta_mrr_global,
    "mrr_friend_a": mrr_a_f, "mrr_friend_d": mrr_d_f, "delta_mrr_friend": delta_mrr_friend,
    "mrr_stranger_a": mrr_a_s, "mrr_stranger_d": mrr_d_s, "delta_mrr_stranger": delta_mrr_stranger,
    "hits_global_a": hits_a_g, "hits_global_d": hits_d_g,
    "hits_friend_a": hits_a_f, "hits_friend_d": hits_d_f,
    "hits_stranger_a": hits_a_s, "hits_stranger_d": hits_d_s,
    "n_features_a": len(cols_A), "n_features_d": len(cols_D),
    "new_features_top20": int(new_in_top20),
    "n_queries": n_q, "n_friend_queries": int(n_f), "n_stranger_queries": int(n_s),
}
pd.to_pickle(results, MODEL_DIR / "lgb_baseline_D_results.pkl")

print("\n" + "=" * 60)
print(f"假设检验完成!")
print(f"  AUC:  A={auc_a:.4f}  D={auc_d:.4f}  Δ={delta_auc:+.4f}")
print(f"  MRR:  A={mrr_a_g:.5f}  D={mrr_d_g:.5f}  Δ={delta_mrr_global:+.5f}")
print(f"        朋友组: A={mrr_a_f:.5f} D={mrr_d_f:.5f} Δ={delta_mrr_friend:+.5f}")
print(f"        陌生人: A={mrr_a_s:.5f} D={mrr_d_s:.5f} Δ={delta_mrr_stranger:+.5f}")
print("=" * 60)
